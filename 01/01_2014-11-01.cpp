...
void Benchmark( int rank, volatile uint8_t * msg, volatile uint8_t * msg2, string filename,
    ...
    uint64_t MSG_MAX_SIZE = 1e7, uint64_t LIN_UP_TO = 4150,
    uint64_t N_LOG_VALUES = 1000, uint64_t NTH_SAVE_DETAILED = 100 )
{
    ...
    while(true) {
        ...
        double t_stddev  = sqrt( (t_sum2-t_sum)/(N*(N-1)) );
        
        if ( rank == 0 ) {
            if (k % NTH_SAVE_DETAILED == 0)
                fprintf( detailedlogfile, "\n" );
            fprintf( logfile, "%lu\t%f\t%f\t%e\t%e\n", length,
                     nc_mean, nc_stddev, t_mean, t_stddev );
#if VERBOSE == 1
            if ( k % NTH_SAVE_DETAILED == 0 ) {
                fflush(logfile);
