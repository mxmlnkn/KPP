...
void Matrix::Clear(void) {                                //delete matrix from heap
    for (int i=0; i<m; i++) {
        delete[] data[i];
    }
    delete[] data;
...
Matrix::Matrix(int m, int n) {                              //Create new 2d-array with (m,n) being the dimension
    Setup(m,n);
    return;
}
...
double& Matrix::operator()(int i, int j) const{
    if (i>=m || j>=n) {
        #if DEBUG == 1
            cout << "Element reference () out of bound\n";
        #endif
        return data[0][0];                                  //TODO: not a nice error-handling, but better than nothing
    }
    return data[i][j];
}
...
Matrix Matrix::operator*(const Matrix &mat) {                   //this * mat
    if (n != mat.m)                                             //Columns of first factor has to be the same amount as rows in 2nd factor
        return *this;
    ...
    for (int i=0; i<m; i++) {                                   //go down every line
        for (int j=0; j<mat.n; j++) {                           //go down every column
            double sum=0;
            for (int k=0; k<m; k++) {                           //go down every column in *this and every line in mat
...
Matrix Matrix::RowEchelon (void) const {             //TODO
    ...
    for (int i=0; i<echelon.m; i++) {
        ...
        double divisor = (abs(echelon(i,i)) == 0) ? (1) : (echelon(i,i));       //abs(divisor), because 0 != -0
...
void Matrix::SetDiagonal(int val) {
    for (int i=0; i<m; i++)
        for (int j=0; j<n; j++)
            if (i==j)
                (*this)(i,i)=val;
...
int Matrix::Load(char* filename) {                      //returns not 0 if there was an error
    ...
    ifstream mf(filename);                              //mf = matrix file
    if (!mf) {
        #if DEBUG == 1
